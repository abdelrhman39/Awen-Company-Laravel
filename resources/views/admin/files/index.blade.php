		...
		<div class="col-12 px-0">
			<div class="col-12 p-0 row">
				<div class="col-12 col-lg-4 py-3 px-3">
					<span class="fas fa-files"></span> {{ __('lang.files manager') }}
				</div>
		...
		<div class="col-12 p-3" style="overflow:auto">
			...
			<table class="table table-bordered  table-hover">
				<thead>
					<tr>
						<th>#</th>
						<th>{{ __('lang.uploader') }}</th>
						<th>{{ __('lang.file') }}</th>
						<th>{{ __('lang.used in') }}</th> 
						<th>{{ __('lang.upload date') }}</th> 
				...
				<tbody>
					...
					<tr>
						...
						<td class="text-truncate" >
							...
							<img src="{{$file->user->getUserAvatar()}}" style="width:50px;height: 50px;border-radius: 50%;display: inline-block;">
							{{$file->user->name_ar}}
							@endif
						</td>
						<td class="text-truncate d-flex">
							...
							@if($file->last_access!=null)
							 <span class="fas fa-clock mx-1"></span>{{ __('lang.last entry') }} {{\Carbon::parse($file->last_access)->diffForHumans()}}
							 <br>
							@endif
						...
						<td style="width: 180px;">
							...
							<form method="POST" action="{{route('admin.files.destroy',$file)}}" class="d-inline-block">@csrf @method("DELETE")
								<button class="btn  btn-outline-danger btn-sm font-1 mx-1 py-1 px-2" onclick="var result = confirm({{ __('lang.are sure of the deleting process ?') }});if(result){}else{event.preventDefault()}">
